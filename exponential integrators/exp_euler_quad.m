...
for time_step = Steps
    ...
    for index = 1:round(T/time_step)
        ...
        % fourriercoefficient of g until at t_old and t_new
            fourcoef_g = func_to_fourrier(func_g(t_old,x), N_val);


        % approximation of the first term
            first_term  = exp(-((pi*(0:N_val-1)).^2)*time_step).* fourcoef_u;

            second_term = fourcoef_g(1,2:end) .* (1 - exp( - substit)) ./ substit;
...
figure("Name","Exp. Euler-Quadratur Fehler", ...
...
legend("C_1 tau  ,C_1 ∊ ℝ " ...
    ,"Exp. Euler Fehler","C_2 tau^2  ,C_2 ∊ ℝ  ","Location","northwest");
